...
public class Exercise05_AcceptPackage {
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches, boolean isSurchargePaid) {
        if ((lengthInches>MAX_DIMENSION_INCHES||widthInches>MAX_DIMENSION_INCHES||heightInches>MAX_DIMENSION_INCHES)&&!isSurchargePaid){
            return false;
        } else if ((lengthInches>MAX_DIMENSION_INCHES||widthInches>MAX_DIMENSION_INCHES||heightInches>MAX_DIMENSION_INCHES)&&isSurchargePaid&&(weightPounds<=MAX_WEIGHT_POUNDS)&&lengthInches*widthInches*heightInches<=MAX_CUBIC_INCHES) {
            return true;
        } else if ((lengthInches<MAX_DIMENSION_INCHES&&widthInches<MAX_DIMENSION_INCHES&&heightInches<MAX_DIMENSION_INCHES)&&!isSurchargePaid&&(weightPounds<=MAX_WEIGHT_POUNDS)&&lengthInches*widthInches*heightInches<=MAX_CUBIC_INCHES) {
            return true;
