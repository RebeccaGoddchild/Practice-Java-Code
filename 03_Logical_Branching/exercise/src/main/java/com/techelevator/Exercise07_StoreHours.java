...
public class Exercise07_StoreHours {
    ...
    public boolean isStoreOpen(int currentHour, char currentDay, boolean isSummer) {
        if(isSummer&&((currentDay=='M'&&currentHour>=8&&currentHour<17)||(currentDay=='W'&&currentHour>=8&&currentHour<20)||(currentDay=='F'&&currentHour>=8&&currentHour<17)||(currentDay=='S'&&currentHour>=9&&currentHour<15))){
            return true;
        } else if (!isSummer&&(currentDay=='M'||currentDay=='W'||currentDay=='F')&&currentHour>=8&&currentHour<17) {
            return true;
        }
        return false;
    }
