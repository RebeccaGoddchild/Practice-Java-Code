...
public class Exercise02_BoardingGate {
    ...
    public boolean[] generateSeatingChart(int numberOfSeats) {
        boolean[] allAvaible = new boolean[numberOfSeats];

        int place = allAvaible.length - 1;
        while (place > -1) {
            allAvaible[place] = AVAILABLE;
            place--;
        }
        return allAvaible;
    }
    ...
    public int getAvailableSeatCount(boolean[] seatingChart) {
        ...
        int place = seatingChart.length - 1;
        while (place > -1) {
            if (seatingChart[place] == AVAILABLE) {
                availableSeats = availableSeats + 1;
                place = place - 1;
            } else if (seatingChart[place] != AVAILABLE) {
                place = place - 1;
            }
        }
    ...
    public int getNumberOfFullRows(boolean[] seatingChart) {
        ...
        int place = seatingChart.length - 1;
        while (place >-1) {
            if (seatingChart[place]==OCCUPIED&&seatingChart[place-1]==OCCUPIED&&seatingChart[place-2]==OCCUPIED) {
                takenRow = takenRow+1;
                place = place -3;
            } else {
                place = place -3;
