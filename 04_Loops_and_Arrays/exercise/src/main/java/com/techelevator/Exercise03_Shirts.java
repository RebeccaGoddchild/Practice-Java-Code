...
public class Exercise03_Shirts {
    ...
    public char[] buildOrder() {
        char[] firstOrder = new char[]{SMALL_TSHIRT, SMALL_TSHIRT, SMALL_TSHIRT, MEDIUM_TSHIRT, MEDIUM_TSHIRT, LARGE_TSHIRT};
        return firstOrder;
    }
    ...
    public char[] buildBulkOrder(int numberOfShirts) {
        ...
        for (int i = 0; i < numberOfShirts; i++) {
            bulkOrder[i] = sizes[i % sizes.length];
        }
    ...
    public boolean placeRequest(char[] order) {
        ...
        int place = order.length - 1;
        while (place > -1) {
            if (order[place] == SMALL_TSHIRT) {
                hasSmall=true;
                place = place - 1;
            } else if (order[place] != SMALL_TSHIRT) {
                place = place - 1;
            }
        }
        return hasSmall;
