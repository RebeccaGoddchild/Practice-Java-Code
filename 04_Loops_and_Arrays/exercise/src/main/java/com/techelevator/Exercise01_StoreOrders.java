...
public class Exercise01_StoreOrders {
    ...
    public int[] createOrder() {
        int[]firstOrder=new int[]{SMALL_CHEESE,CALZONE,LARGE_PEPPERONI,SPAGHETTI_PIE};
        return firstOrder;
    }
    ...
    public int getCalzoneSales(int[] orders) {
        int calzoneSales=0;
        int place= orders.length-1;
        while (place>-1){
            if(orders[place] == CALZONE){
                calzoneSales=calzoneSales+1;
                place=place-1;
            } else if (orders[place] != CALZONE) {
                place=place-1;
            }
        }return calzoneSales;
    }
    ...
    public int getCheesePizzaRevenue(int[] orders) {
        ...
        int place= orders.length-1;
        while (place>-1){
            if(orders[place] ==SMALL_CHEESE){
                cheesePizzaRevenue=cheesePizzaRevenue+8;
                place=place-1;
            } else if (orders[place] == MEDIUM_CHEESE) {
                cheesePizzaRevenue=cheesePizzaRevenue+11;
                place=place-1;
            } else if (orders[place] ==LARGE_CHEESE) {
                cheesePizzaRevenue=cheesePizzaRevenue+14;
                place=place-1;
            } else if ((orders[place] !=SMALL_CHEESE)|| (orders[place] !=MEDIUM_CHEESE)||(orders[place] !=LARGE_CHEESE)){
                place=place-1;
            }
